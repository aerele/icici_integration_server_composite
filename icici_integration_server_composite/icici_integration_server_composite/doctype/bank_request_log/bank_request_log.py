...
def get_encrypted_request_aysnc(data, connector_doc):
...
def get_decrypted_response_aysnc(response, connector_doc):
	...
	rsa_key = RSA.importKey(open(private_key_path, "rb").read())
...
def get_bank_balance(payload):
	...
	data = {
		...
		"URN":connector_doc.urn,
		"ACCOUNTNO": payload.bank_account_number
	}

	res_dict = frappe._dict({})

	bank_request_log_doc = frappe.new_doc("Bank Request Log")
	bank_request_log_doc.config_details = json.dumps(payload, indent=4)
	bank_request_log_doc.payload = json.dumps(data, indent=4)
	bank_request_log_doc.api_method = "get_bank_balance"
	bank_request_log_doc_name = bank_request_log_doc.insert().name
	frappe.db.commit()

	try:
		response = requests.post(bank_balance_url, headers=headers, data=get_encrypted_request_aysnc(data, connector_doc))
		frappe.db.set_value("Bank Request Log", bank_request_log_doc_name, "status_code", response.status_code)
		if response.ok:
			try:
				decrypted_response = get_decrypted_response_aysnc(response, connector_doc)
				frappe.db.set_value("Bank Request Log", bank_request_log_doc_name, "response", json.dumps(decrypted_response))
			...
			frappe.db.set_value("Bank Request Log", bank_request_log_doc_name, "response", json.dumps(response.text))
...
def encrypt_data(data, connector_doc):
	public_key_path = frappe.get_doc("File", {"file_url": connector_doc.bank_public_key}).get_full_path()
...
def decrypt_data(data, key):
	...
	cipher= AES.new(key, AES.MODE_CBC, message[:16])
...
def get_decrypted_response(response):
	if response:
		response=json.loads(response.text)
...
def get_bank_statement(payload):
	try:
		...
		data = {
			...
			"CONFLG": payload.conflg if payload.conflg else "N"
		}
		...
		bank_request_log_doc.config_details = json.dumps(payload, indent=4)
		bank_request_log_doc.payload = json.dumps(data, indent=4)
		bank_request_log_doc.api_method = "get_bank_statement"
		bank_request_log_doc_name = bank_request_log_doc.insert().name
		frappe.db.commit()

		encrypted_data = encrypt_data(json.dumps(data).encode("utf-8"), connector_doc)

		response = requests.post(bank_statement_url, headers=headers, data=encrypted_data)
		frappe.db.set_value("Bank Request Log", bank_request_log_doc_name, "status_code", response.status_code)
		if response.ok:
			try:
				...
				if decrypted_response:
					...
					if "Record" in decrypted_response:
						res_dict.res_text = decrypted_response
						res_dict.res_status = response.status_code
				...
				res_dict.server_status="Failed"
				res_dict.res_text = response.text
				frappe.log_error(title="get_bank_statement - API Response", message= frappe.get_traceback())
			...
			res_dict.res_text = response.text
			res_dict.res_status = response.status_code
			res_dict.api_method = "get_bank_statement"
			res_dict.config_details = data
			res_dict.server_status="Failed"
	except:
		res_dict.api_method = "get_bank_statement"
...
@frappe.whitelist()
def get_bank_statements_paginated(payload):
	try:
		if not frappe.has_permission("Bank Request Log", "write"):
			frappe.throw("Not permitted", frappe.PermissionError)
		...
		if payload.last_tran_id:
			data.update({"LASTTRID": payload.last_tran_id })
		...
		bank_request_log_doc.config_details = json.dumps(payload, indent=4)
		bank_request_log_doc.payload = json.dumps(data, indent=4)
		bank_request_log_doc.api_method = "get_bank_statements_paginated"
		bank_request_log_doc_name = bank_request_log_doc.insert().name
		frappe.db.commit()

		encrypted_data = encrypt_data(json.dumps(data).encode("utf-8"), connector_doc)

		response = requests.post(bank_statement_url_paginated, headers=headers, data=encrypted_data)
		frappe.db.set_value("Bank Request Log", bank_request_log_doc_name, "status_code", response.status_code)
		if response.ok:
			try:
				decrypted_response = get_decrypted_response(response)
				...
				res_dict.server_status="Failed"
				res_dict.res_text = response.text
				frappe.log_error(title="get_bank_statements_paginated - API Response", message= frappe.get_traceback())
			...
			res_dict.res_text = response.text
			res_dict.res_status = response.status_code
			res_dict.api_method = "get_bank_statements_paginated"
			res_dict.config_details = data
			res_dict.server_status="Failed"
	except:
		res_dict.api_method = "get_bank_statement"
